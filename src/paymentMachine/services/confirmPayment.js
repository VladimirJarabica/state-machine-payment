...
const CONFIRM_PAYMENT_API =
  "https://skymock.skypicker.com/api/v0.1/confirm_payment";

export const confirmPayment = async (context, event) => {
  const response = await fetch(CONFIRM_PAYMENT_API, {
    method: "POST",
    body: JSON.stringify({
      paymentCvv: "123",
      paymentMethodToken: context.saveBookingResponseData.auth_token,
      paymentToken: context.saveBookingResponseData.zooz_token,
      sandbox: context.saveBookingResponseData.sandbox
    }),
    headers: {
